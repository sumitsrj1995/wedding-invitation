...
const WRAP_CLASS = 'theme4-photo-depth';
...
function wrapGalleryImage(card) {
  ...
  shadow.className = 'theme4-photo-depth__shadow';
  shadow.setAttribute('aria-hidden', 'true');

  const frame = document.createElement('div');
  frame.className = 'theme4-photo-depth__frame';

  const bgLayer = document.createElement('div');
  bgLayer.className = 'theme4-photo-depth__layer theme4-photo-depth__layer--bg';
  ...
  fgLayer.className = 'theme4-photo-depth__layer theme4-photo-depth__layer--fg';
  fgLayer.appendChild(img);

  const shine = document.createElement('div');
  shine.className = 'theme4-photo-depth__shine';
  shine.setAttribute('aria-hidden', 'true');

  const vignette = document.createElement('div');
  vignette.className = 'theme4-photo-depth__vignette';
...
function initPhotoDepth(shell) {
  ...
  const update = () => {
    ...
    cards.forEach((card, index) => {
      ...
      card.style.setProperty('--t4-scroll-shift', `${scrollShift.toFixed(2)}px`);
      card.style.setProperty('--t4-bg-y', `${bgShift.toFixed(2)}px`);
      card.style.setProperty('--t4-fg-y', `${fgShift.toFixed(2)}px`);
      card.style.setProperty('--t4-tilt-x', `${tiltX.toFixed(2)}deg`);
      card.style.setProperty('--t4-tilt-y', `${tiltY.toFixed(2)}deg`);
      card.style.setProperty('--t4-lift', `${lift.toFixed(2)}px`);
      card.style.setProperty('--t4-shine-x', `${50 + mouseX * 12}%`);
      card.style.setProperty('--t4-shine-y', `${44 + mouseY * 10}%`);
      card.style.setProperty('--t4-shadow-opacity', `${0.42 + Math.abs(mouseY) * 0.08}`);
    });
  };
...
export default function PhotoDepthEnhancer() {
  useEffect(() => {
    ...
    const attach = () => {
      if (cleanup) return;
      const shell = document.querySelector('html[data-theme="theme4"] .gallery-deck-shell');
      if (!shell) return;
