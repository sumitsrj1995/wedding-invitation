...
const WRAP_CLASS = 'theme8-photo-depth';
...
function wrapGalleryImage(card) {
  ...
  shadow.className = 'theme8-photo-depth__shadow';
  shadow.setAttribute('aria-hidden', 'true');

  const frame = document.createElement('div');
  frame.className = 'theme8-photo-depth__frame';

  const bgLayer = document.createElement('div');
  bgLayer.className = 'theme8-photo-depth__layer theme8-photo-depth__layer--bg';
  ...
  fgLayer.className = 'theme8-photo-depth__layer theme8-photo-depth__layer--fg';
  fgLayer.appendChild(img);

  const shine = document.createElement('div');
  shine.className = 'theme8-photo-depth__shine';
  shine.setAttribute('aria-hidden', 'true');

  const vignette = document.createElement('div');
  vignette.className = 'theme8-photo-depth__vignette';
...
export default function PhotoDepthEnhancer() {
  useEffect(() => {
    ...
    const attach = () => {
      if (cleanup) return;
      const shell = document.querySelector('html[data-theme="theme8"] .gallery-deck-shell');
      if (!shell) return;
